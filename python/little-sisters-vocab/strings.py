...
def make_word_groups(vocab_words):
    ...
    """
    made_words = [vocab_words[0]]

    for word in vocab_words[1:]:
        made_words.append(vocab_words[0] + word)    
    return ' :: '.join(made_words)
...
def remove_suffix_ness(word):
    ...
    if word_without_suffix[-1] == 'i':
        word_without_suffix = word_without_suffix.replace('i', 'y');
...
def adjective_to_verb(sentence, index):
    ...
    words[index] = words[index].replace(words[index], words[index] + 'en')
    if '.' in words[index]:
